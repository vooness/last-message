...
export async function uploadToS3(
  ...
  try {
    ...
    const command = new PutObjectCommand({
      ...
      // ŽÁDNÉ ACL - bucket zůstává privátní
    ...
    // Vrátíme jen key, ne URL - URL se generuje až při zobrazení
    return key
  } catch (error) {
 ...
 * POZOR: Tato funkce je EXPORTOVÁNA
 */
export async function getPresignedVideoUrl(key: string, expiresIn: number = 7 * 24 * 3600): Promise<string> {
  try {
    ...
    const presignedUrl = await getSignedUrl(s3Client, command, { 
      expiresIn // URL vyprší za 7 dní (default)
    })
